...
class Event(Resource):
    ...
    id = db.Column(db.Integer, db.ForeignKey('resources.id'), primary_key=True)
    category = db.Column(db.Enum(CategoryEnum), nullable=True)  # 'sport', 'evenement', 'vie-associative', 'soiree'
    # dépendance circulaire entre table à migrer séparemment
    cover_image_id = db.Column(db.Integer, db.ForeignKey('files.id'), nullable=True) # photo de couverture du dossier
