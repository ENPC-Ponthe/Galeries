...
class GalleryDAO(ResourceDAO):
    ...
    @staticmethod
    def find_all_public_sorted_by_date(page, page_size):
        return Gallery.query.filter_by(private=False).order_by(desc(Gallery.created)).offset((page-1)*page_size).limit(page_size).all()
