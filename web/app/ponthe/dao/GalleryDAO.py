...
class GalleryDAO(ResourceDAO):
    ...
    def find_all_public_sorted_by_date_filtered_by_years(beginning_year, ending_year):
        return Gallery.query.join(Gallery.year).filter_by(private=False).where(between(Gallery.year, beginning_year, ending_year)).order_by(desc(Gallery.created)).all()
