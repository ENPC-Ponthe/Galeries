...
from ...file_helper import is_allowed_file, get_base64_encoding, is_image, is_video, is_archive
    ...
    def post(self, gallery_slug):
        ...
        if file and is_allowed_file(file.filename):
            if is_image(file.filename):
                ...
                FileService.save_video_in_all_resolutions(
                    file, gallery_slug, current_user)
            elif is_archive(file.filename):
    ...
    def post(self):
        ...
        for year in year_list:
            ...
            for gallery in public_galleries:
                list_of_files = list(
                    filter(lambda file: not file.pending, gallery.files))
                if list_of_files:
...
class GetAllGalleries(Resource):
    ...
    def post(self):
        ...
        '''Get the list of public galleries of all years'''
        gallery_list = []
        public_galleries = GalleryDAO().find_all_public_photo_sorted_by_date(
            page, page_size, starting_year, ending_year)
        number_of_public_image_galleries = GalleryDAO(
        ).count_all_public_photo_sorted_by_date(starting_year, ending_year)
...
class GetGalleries(Resource):
    ...
    def get(self, event_slug: str):
        ...
        for year, galleries in galleries_by_year.items():
            gby_dict[year.slug] = [gallery.serialize()
...
class GetImages(Resource):
    ...
    def post(self, gallery_slug: str):
        ...
        if GalleryDAO.has_right_on(gallery, current_user):
            list_of_files = FileDAO.find_all_files_by_gallery(
                gallery, page, page_size)
            total_number_of_files = FileDAO.get_number_of_files_by_gallery(
                gallery)
        else:
            list_of_files = FileDAO.find_not_pending_files_by_gallery(
                gallery, page, page_size)
            total_number_of_files = FileDAO.get_number_of_not_pending_files_by_gallery(
                gallery)
        ...
        for file in list_of_files:
            encoded_file = FileService.get_base64_encoding_thumb(
                file, SIZE_LARGE_THUMB)
        ...
        for i in range(len(list_of_files)):
            ...
            own_reaction_type = ReactionService.get_user_reaction_type_by_slug(
                file_slug, current_user)
            all_reactions = ReactionService.count_reactions_by_image_slug(
                file_slug)
            approved_file = {
                'file_path': list_of_files[i].file_path,
...
class GetLatestGalleries(Resource):
    ...
    def post(self):
        ...
        public_galleries = GalleryDAO().find_all_public_galleries_sorted_by_date(
            page, page_size, starting_year, ending_year)
